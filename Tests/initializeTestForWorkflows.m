classdef initializeTestForWorkflows < matlab.unittest.TestCase
    ...
    methods (TestMethodSetup)
        ...
        function listOpenLibrary(test)
            % List all open simulink models
            test.openLibrariesBefore = get_param(Simulink.allBlockDiagrams('library'),'Name');
        end
    ...
    methods(TestMethodTeardown)
        ...
        function closeOpenedLibraries(test)
            % Close all libraries opened during test
            openLibrariesAfter = get_param(Simulink.allBlockDiagrams('library'),'Name');
            librariesOpenedByTest = setdiff(openLibrariesAfter, test.openLibrariesBefore);
            close_system(librariesOpenedByTest, 0);
        end
